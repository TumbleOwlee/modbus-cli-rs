...
impl Client {
    ...
    pub async fn run(&mut self, delay_after_connect: usize, interval_ms: usize, timeout_ms: usize) {
        ...
        tokio::time::sleep(tokio::time::Duration::from_millis(
            delay_after_connect as u64,
        ))
        .await;

        let mut time_last_read = SystemTime::now()
            .checked_sub(Duration::from_millis(interval_ms as u64 + 1))
            .unwrap();
        let mut op_idx = 0;
        let mut retries = 0;
        loop {
