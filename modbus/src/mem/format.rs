...
impl Format {
    ...
    pub fn as_plain_str(&self, bytes: &[u16]) -> anyhow::Result<String> {
        match self.format {
            ...
            FormatType::PackedAscii => String::from_utf8(
                bytes
                    .iter()
            ...
            .map_err(|e| e.into()),
            FormatType::LooseAscii => String::from_utf8(
                bytes
                    .iter()
                    .map(|v| self.apply_order(*v))
                    .map(|v| (v & 0xFF) as u8)
            ...
            .map_err(|e| e.into()),
            FormatType::PackedUtf8 => String::from_utf8(
                bytes
                    .iter()
                    .map(|v| self.apply_order(*v))
    ...
    pub fn encode(&self, s: &str) -> anyhow::Result<Vec<u16>> {
        match self.format {
            ...
            FormatType::LooseAscii => Ok(s.chars().map(|c| self.apply_order(c as u16)).collect()),
            FormatType::LooseUtf8 => Ok(s.encode_utf16().map(|c| self.apply_order(c)).collect()),
            FormatType::PackedUtf8 => {
                let mut v = Vec::with_capacity(s.len() / 2 + 1);
            ...
            FormatType::U16 | FormatType::U16le => {
            ...
            FormatType::I16 | FormatType::I16le => {
