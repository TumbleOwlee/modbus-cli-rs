...
where
    T: Hash + Debug + PartialEq + Eq + Clone + Default + Send + Sync + 'static,
{
    pub fn with_tcp_client(config: ClientConfig<T, modbus_net::tcp::Config>) -> Self {
        Self {
            builder: Builder::TcpClient(modbus_net::tcp::ClientBuilder::new(
                config.id,
                config.config,
                config.operations,
                config.memory,
    ...
    pub fn with_rtu_client(config: ClientConfig<T, modbus_net::rtu::Config>) -> Self {
        Self {
            builder: Builder::RtuClient(modbus_net::rtu::ClientBuilder::new(
                config.id,
                config.config,
                config.operations,
                config.memory,
    ...
    pub fn with_tcp_server(config: ServerConfig<T, modbus_net::tcp::Config>) -> Self {
        Self {
            builder: Builder::TcpServer(modbus_net::tcp::ServerBuilder::new(
                config.id,
                config.config,
                config.memory,
            )),
    ...
    pub fn with_rtu_server(config: ServerConfig<T, modbus_net::rtu::Config>) -> Self {
        Self {
            builder: Builder::RtuServer(modbus_net::rtu::ServerBuilder::new(
                config.id,
                config.config,
                config.memory,
            )),
    ...
    pub async fn send_command(&self, command: modbus_net::Command) -> Result<(), Error> {
        if self.handle.is_none() {
            return Err(InstanceError::NotRunning.into());
        }
        match &self.handle {
