...
| **UI-E-147** | Badge on a file node that carries no status (UI-R-315, UI-R-321) | the name is drawn with no leading status marker and the badge still follows it one space later; the name keeps the normal text style, and the badge takes the style its type reports (UI-R-316) or that same normal text style when its type reports none (UI-R-322) |
...
| **UI-E-149** | Badge text widening a row past the file tree's area (UI-R-315, UI-E-106) | the row is clipped at the area width as any other row is; because the badge follows the name, the badge is what loses cells and can be clipped away entirely, and the file tree still never scrolls horizontally |
| **UI-E-150** | Badge type reporting an empty text for a node's badge (UI-R-314) | the row renders as an unbadged row (UI-R-318): no cells and no separating space are drawn for it, whatever style that badge reports |
...
| **UI-E-066** | Tab widget rendered with an active index out of range for its tab list (UI-R-119) | the index reads as `0` (UI-R-325), so the first tab takes the active style (UI-R-116) and the centring scroll computes its window from that tab's block (UI-R-118) exactly as for any in-range active tab; an empty tab list draws nothing per UI-E-065; never panics |
