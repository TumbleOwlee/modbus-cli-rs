...
| **UI-E-147** | Badge on a file node that carries no status (UI-R-315, UI-R-321) | the badge marker is drawn immediately before the name with no status marker preceding it; the name keeps the normal text style and the badge keeps its own (UI-R-316) |
...
| **UI-E-149** | Badge marker widening a row past the file tree's area (UI-R-315, UI-E-106) | the row is clipped at the area width as any other row is; because the badge precedes the name, the name is what loses cells, and the file tree still never scrolls horizontally |
