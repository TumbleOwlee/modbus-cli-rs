...
| **UI-E-147** | Badge on a file node that carries no status (UI-R-315, UI-R-321) | the name is drawn with no leading status marker and the badge still follows it one space later; the name keeps the normal text style and the badge keeps its own (UI-R-316) |
...
| **UI-E-149** | Badge marker widening a row past the file tree's area (UI-R-315, UI-E-106) | the row is clipped at the area width as any other row is; because the badge follows the name, the badge is what loses cells and can be clipped away entirely, and the file tree still never scrolls horizontally |
| **UI-E-150** | Badge whose display marker is the empty string (UI-R-314) | the row renders as an unbadged row (UI-R-318): no cells and no separating space are drawn for it |
